...
export const analyzeRepo = async (repoUrl: string) => {
    ...
    try {
        ...
        if (!Array.isArray(data) && "content" in data) {
            ...
            const response = await fetch('http://localhost:8080/api/ai/analyze-tech', {
                method: 'POST',
                headers: { 
            ...
            console.log(`Đây là nội dung file package.json, hãy phân tích các công nghệ sử dụng trong này: ${decodedContent}`)
