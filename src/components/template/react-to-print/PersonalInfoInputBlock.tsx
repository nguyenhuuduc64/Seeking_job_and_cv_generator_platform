...
const PersonalInfoInputBlock: React.FC<Props> = ({ data, onDataChange, onDelete }) => {
    ...
    const [localData, setLocalData] = useState<PersonalInfoData>(
        data || {
            fullName: t('cv.personal.default_name'),
            position: t('cv.personal.default_position'),
            dob: 'DD/MM/YYYY',
            gender: t('cv.personal.default_gender'),
            phone: '0123 456 789',
            email: 'duchuuiq@gmail.com',
            website: 'facebook.com',
            address: t('cv.personal.default_address'),
            avatarUrl:
    ...
    const editor = useBlockEditor(
        localData.fullName || 'Nguyen duc',
        t('cv.personal.name_placeholder'),
        (html) => {
    ...
    const editorPosition = useBlockEditor(
        localData.position || t('cv.personal.default_position'),
        t('cv.personal.default_position'),
        (html) => {
            const newData = { ...localData, position: html };
            setLocalData(newData);
            if (onDataChange) onDataChange(newData);
        }
    );
    ...
    if (!editor || !editorPosition) return null;
    ...
    return (
        <div className="group relative mb-10 flex w-full flex-col gap-6 bg-white p-4 md:flex-row print:flex-row print:gap-8 print:mb-6">
            {/* Toolbar chỉ hiện khi hover thưa ông chủ */}
            ...
            <div className="flex flex-1 flex-col gap-1">
                {/* Họ tên */}
                <div className="prose prose-2xl max-w-none text-black font-bold leading-tight">
                    <EditorContent editor={editor} />
                </div>

                {/* Vị trí ứng tuyển */}
                <div className="prose prose-lg max-w-none text-gray-500 italic leading-tight mb-4">
                    <EditorContent editor={editorPosition} />
                </div>

                <div className="flex flex-col gap-1.5">
                    {[
                        { label: t('cv.personal.dob'), field: 'dob' },
                        { label: t('cv.personal.gender'), field: 'gender' },
                        { label: t('cv.personal.phone'), field: 'phone' },
                        { label: t('cv.personal.email'), field: 'email' },
                        { label: t('cv.personal.address'), field: 'address' },
                    ].map((item) => (
                        <div
                            key={item.field}
                            className="flex items-start border-b border-transparent"
                        >
                            <span className="w-28 text-sm font-bold text-black shrink-0 py-1">
                                {item.label}
                            </span>
                            <TextareaAutosize
                            ...
                            />
                            <p className={printTextStyle}>{(localData as any)[item.field]}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            {onDelete && (
                <button
                    onClick={onDelete}
                    className="no-print absolute -right-2 -top-2 hidden group-hover:block bg-red-500 rounded-full p-1 text-white shadow-lg"
                >
