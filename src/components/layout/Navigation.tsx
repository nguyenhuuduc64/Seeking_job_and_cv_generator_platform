...
export function Navigation() {
    ...
    const getMenuItems = () => {
        ...
        if (role === 'admin') return [...base, { name: 'Quản trị hệ thống', onClick: () => navigate('/admin') }];
        if (role === 'recruiter') return [...base, { name: 'Quản lý tuyển dụng', onClick: () => navigate('/tuyen-dung') }];
                ...
                <div className="flex items-center gap-2 lg:gap-6">
                                ...
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <Button variant="ghost" className="flex items-center gap-2 px-2 focus-visible:ring-0">
                                            <span className="font-bold text-gray-800 uppercase text-sm">{currentUser.fullName}</span>
                                    ...
                                    <DropdownMenuContent className="w-56 mt-2 rounded-sm " align="end">
                                        ...
                                        {menuItems.map((item, index) => (
                                            <DropdownMenuItem key={index} onClick={item.onClick} className="cursor-pointer font-bold text-xs uppercase py-2">{item.name}</DropdownMenuItem>
                                        ))}
                                        <DropdownMenuSeparator />
                                        <DropdownMenuItem onClick={handleLogout} className="cursor-pointer font-bold text-xs uppercase py-2 text-red-600 focus:text-red-600">Đăng xuất</DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                                <div className="flex items-center space-x-3 border-l-2 border-gray-200 pl-4 ml-2">
                                    <div className="flex flex-col">
                                        <p className="text-gray-500 text-[10px] font-light italic leading-none">Nhà tuyển dụng?</p>
                                        <a href="/dang-ky-tuyen-dung" className="font-bold text-[12px] hover:text-green-600 mt-1" style={{ color: 'var(--primary-color)' }}>Đăng ký ngay</a>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <div >
                        <Sheet>
                            ...
                            <SheetContent side="right" className="w-[300px] sm:w-[400px] overflow-y-auto z-5000">
                                ...
                                <div className="flex flex-col gap-2 mt-6">
                                    ...
                                    <Accordion type="single" collapsible className="w-full">
                                        <AccordionItem value="jobs" className="border-none">
                                            ...
                                            <AccordionContent className="flex flex-col gap-4 pl-4 pt-2 pb-4">
                                                <Link to="/jobs" className="text-blue-600 font-bold uppercase text-sm border-l-2 border-blue-600 pl-3">
                                                    Tất cả việc làm
                                                </Link>
                                                <Link to="/categories" className="text-gray-600 font-bold uppercase text-sm border-l-2 border-gray-200 pl-3">
                                                    Ngành nghề
                                                </Link>
                                            </AccordionContent>
                                        </AccordionItem>
                                        ...
                                        <div className="py-3 border-b">
                                            <Link to="/companies" className="text-lg font-bold uppercase block">
                                                Công ty
                                            </Link>
                                        </div>

                                        <div className="py-3 border-b">
                                            <Link to="/candidates" className="text-lg font-bold uppercase block">
                                                Ứng viên
                                            </Link>
                                        </div>
                                    </Accordion>
                                        ...
                                        {!currentUser ? (
                                            ...
                                            <div className="flex flex-col gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                                                <div className="flex flex-col mb-2">
                                                    <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest">Xin chào thưa ông chủ</p>
                                    ...
                                    <div className="mt-10 p-6 bg-blue-50 rounded-xl border-2 border-dashed border-blue-200 text-center">
                                        <p className="text-gray-600 text-xs font-medium mb-2">Bạn là nhà tuyển dụng?</p>
                                        <a
...
function ListItem({ title, children, href }: { title: string; children: string; href: string }) {
    return (
        <li>
            <NavigationMenuLink asChild>
                <Link to={href} className="block select-none space-y-1 rounded-sm p-3 leading-none no-underline outline-none transition-colors hover:bg-gray-100">
                    <div className="text-sm font-bold uppercase text-blue-600 tracking-tight">{title}</div>
                    <p className="line-clamp-2 text-xs leading-snug text-muted-foreground mt-1 italic">{children}</p>
                </Link>
            </NavigationMenuLink>
        </li>
    )
