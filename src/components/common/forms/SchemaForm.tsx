...
export const SchemaFormField: React.FC<{ field: FieldConfig; parentName?: string }> = ({
    ...
    if (field.children && field.children.length > 0) {
        return (
            <div className={`space-y-4 ${field.className || ''} border p-4 rounded-md bg-gray-50`}>
                {field.label && (
                    <h3 className="font-semibold text-lg text-gray-700 mb-2">{field.label}</h3>
                )}
    ...
    const commonClasses =
        'w-full p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors';
...
export const SchemaForm: React.FC<SchemaFormProps> = ({
    ...
    const openFormName = useAppSelector((state) => state.form.openFormName);
        ...
        <FormProvider {...methods}>
            <div className="fixed inset-0 z-[999] bg-black/50 backdrop-blur-sm">
                <form
                    ...
                    className={`max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 bg-white shadow-md rounded-lg p-6 fixed z-100 w-1/3 mx-auto top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 ${
                        className || ''
                    }`}
                >
                    <div className="flex justify-end" onClick={handleCloseForm}>
                        <FontAwesomeIcon
                            icon={faClose}
                            className="text-lg w-12 h-12 cursor-pointer"
                        />
                    </div>
